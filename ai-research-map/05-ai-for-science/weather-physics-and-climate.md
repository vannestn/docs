...
- **Physics foundation models** — an emerging push toward a single model that simulates many
  PDE (partial differential equation) systems from a *prompt* of prior states, with no
  per-system retraining — echoing the LLM "train once, deploy anywhere" paradigm. *"Towards a
  Physics Foundation Model"* introduces **GPhyT** (see below).
  [arXiv:2509.13805](https://arxiv.org/abs/2509.13805)
...
narrowly-scoped system and must be retrained or fine-tuned for any new PDE or boundary
condition. The authors argue the path to a true Physics Foundation Model (PFM) is to *emulate
the in-context learning of LLMs*: infer the governing dynamics from a short sequence of prior
...
**Mechanism.** A hybrid of deep learning and classical numerics. A transformer-based **neural
...
**numerical integrator** (first-order Forward Euler here; Heun/RK4 possible) then advances the
state via X_{t+1} = f(X_t, ∂X/∂t|_t, Δt). Two physics-motivated choices matter: (1) the model
predicts the *derivative* rather than the next state directly, and (2) the input is augmented
with **numerically-computed spatial (∂x, ∂y) and temporal (∂t) derivatives** of the fields,
concatenated along the channel dimension to help resolve sharp gradients and shocks.
...
- *In-context / zero-shot generalization:* on novel boundary conditions and entirely novel
  ...
  beats simply predicting the image mean) — evidence that it infers new dynamics from the
  prompt alone, without fine-tuning.
...
long-term stability still far short of numerical solvers, with error accumulation that looks
inherent to all neural rollouts tested; coverage limited to fluid dynamics and heat transfer
(no mechanics, chemistry, molecular dynamics, or optics); fixed 256×128 resolution (variable
resolution is future work). The authors frame GPhyT as a step *toward*, not an arrival at, a PFM.

## State of research

**Best-performing now:** ML weather forecasting is *mature and winning* — diffusion
ensembles (GenCast) and graph/operator forecasters beat operational NWP (numerical weather
prediction) on most variables at a fraction of the compute (8 min on one TPU vs. hours on a
supercomputer), and are now in production (WeatherNext 2 powers Google Search/Maps/Gemini
forecasts).

**Promising but unproven:** General-purpose **physics foundation models** (one model across
many PDEs and domains) are early — most success is still domain-specific. GPhyT is the most
direct demonstration that a single transformer can learn disparate physics from data alone and
infer unseen dynamics in-context, but it remains an early step rather than a finished PFM.

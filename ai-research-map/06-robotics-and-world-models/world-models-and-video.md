...
- **Genie 3** (DeepMind) — generates interactive, navigable 3D worlds from text via
  autoregressive frame generation, in real time at 24fps / 720p. Holds ~1 minute of visual
  memory (objects stay coherent for about a minute if they leave and re-enter frame) and
  shows emergent physical consistency; supports promptable world events and powers SIMA
  agent training. [DeepMind](https://deepmind.google/blog/genie-3-a-new-frontier-for-world-models/)
- **World Labs (Fei-Fei Li)** — **Marble** (persistent, navigable 3D environments from text,
  ...
  Taxonomy of World Models" (Jun 2026) — a conceptual stake in the field — argues the term
  conflates three distinct system types: a **renderer** (outputs pixels for human eyes, where
  visual fidelity is what matters), a **simulator** (outputs computable, physically and
  geometrically faithful *state*), and a **planner**.
  [Taxonomy](https://drfeifei.substack.com/p/a-functional-taxonomy-of-world-models)
- **Decart Oasis 3** — real-time interactive world model for photorealistic driving sim,
  served via API (three camera views, <200ms end-to-end latency, ~$0.02/sec). Acknowledged
  caveats: scene consistency drifts over time, and physical interactions (e.g. collisions)
  are sometimes mis-modeled.
...
- **Sora 2** (OpenAI) and **Veo 3** (Google) — markedly improved physics (gravity, momentum,
  collisions, fluids) plus synchronized audio; OpenAI positions Sora 2 as a step toward a
  "general-purpose world simulator." Physical fidelity is better but still approximate.
- **NVIDIA Cosmos-Predict2.5 / Cosmos-Transfer2.5** — open world foundation models for
  Physical AI (synthetic data plus closed-loop simulation for robotics/AV). A flow-matching
  DiT unifying Text2World / Image2World / Video2World in one model, released at **2B and 14B**;
...
**Mechanism.** Two stages. (1) *Action-free pretraining*: a ViT encoder plus predictor is
trained with a mask-denoising objective in *learned representation space* — it predicts the
representation of masked video patches, not their pixels — using an L1 loss against an EMA
...
post-training*: freeze the encoder and train **V-JEPA 2-AC**, a ~300M-parameter block-causal
transformer predictor, on just **62 hours** of unlabeled Droid robot video; it learns to
autoregressively predict the next-frame representation given the end-effector action and
...
imagined future representation and a goal-image representation, execute the first action, then
...
- **Zero-shot robot manipulation** on Franka arms in two labs not seen in training:
...
specification* is image-based (language goals are future work). (4) Epic-Kitchens-100
anticipation is limited to a fixed kitchen vocabulary.
...
**Mechanism.** A **flow-matching** DiT (replacing the EDM-diffusion of Cosmos-Predict1) that
unifies **Text2World, Image2World, and Video2World** in a single model via a frame-replacement
...
pipeline: flow-matching pretraining → domain-specific supervised fine-tuning (object
permanence, high motion, complex scenes, driving, manipulation) → **model merging** (a model
soup) → a GRPO-style **RL** post-train against a VLM reward model (VideoAlign).
...
- **Downstream robotics:** a diffusion policy trained on Cosmos-Transfer2.5-augmented
  observations hits **24/30** successes across 10 real-robot test scenarios, vs. 5/30
  (standard image augmentation) and 1/30 (base). Action-conditioned generation on Bridge beats
  the Cosmos-Predict1 baseline on all metrics (PSNR 24.95 vs. 21.14; FVD 146 vs. 190), with an
...
For *robotics planning*, V-JEPA-2-style predictive models are the working approach — notably,
on real-robot interaction tasks V-JEPA 2-AC outplans a Cosmos latent-diffusion world model at
~15× lower per-action latency, an early data point in the generative-vs-predictive debate.
For *synthetic data and closed-loop sim*, Cosmos-Transfer2.5 shows that pixel-space generators
can measurably improve downstream robot policies.

**Promising but unproven:** Whether generated worlds are *consistent and controllable* enough
to serve as true simulators (for training agents, AV, robotics) rather than just impressive
visuals. Fei-Fei Li's renderer/simulator/planner taxonomy frames the open question precisely:
a *renderer* optimizes pixels for human eyes, while a *simulator* must output computable,
faithful *state* — and most "world model" demos are the former being sold as the latter.
...
- **Long-horizon consistency and memory** — generated worlds drift after seconds–minutes
  ...
  short horizons.
- **True physics** — still approximate or hallucinated; collisions and contact are where both
...
- **Evaluation** — no agreed metric. Cosmos resorts to a custom benchmark suite (PAI-Bench)
  ...
  field hasn't even settled on what "world model" *means*.

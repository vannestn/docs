...
- **Mistral AI** (France) — Europe's leading frontier lab. **Mistral Large 3** (sparse MoE,
  ...
  **Magistral**, Mistral's first reasoning model. Magistral Medium was trained for reasoning
  on top of Mistral Medium 3 with **RL alone** — no distillation from a teacher — and the
  RLVR pipeline alone yielded a ~50% absolute jump on AIME-24 (pass@1) over the base
...
- **Kyutai** (France) — open-science real-time speech. **Moshi**, a full-duplex speech-text
  foundation model built on the 7B **Helium** text LLM, with **Mimi** (a streaming RVQ neural
  audio codec) and an "Inner Monologue" scheme that predicts time-aligned text tokens as a
  prefix to audio. Theoretical latency 160ms (200ms in practice).
  [Moshi arXiv:2410.00037](https://arxiv.org/abs/2410.00037)

## Sovereign efforts

- **UAE — TII**: **Falcon-H1**, a *parallel* hybrid where attention and Mamba-2 SSM heads run
  side-by-side inside each block and their outputs are concatenated (the attention/SSM channel
  split is tunable). Family spans 0.5B → 34B with 256K context and 18 languages. The flagship
  **Falcon-H1-34B-Instruct** rivals/outperforms 70B-class open models (Qwen2.5-72B,
  Llama-3.3-70B) at roughly half the size; the 0.5B matches typical 2024-era 7B models.
...
- **Japan — Sakana AI**: evolutionary model-merging. **M2N2** ("Model Merging of Natural
  Niches", GECCO '25) evolves *merging boundaries* (split-points) rather than fixing them to
  layers, adds an implicit-fitness-sharing **competition** term for diversity, and an
  **attraction** heuristic to pick complementary parent pairs. It can evolve models *from
  scratch* (matching CMA-ES on MNIST far more cheaply) and merges specialist LLMs / diffusion
  models — e.g. fusing a Japanese-prompt SDXL with English-prompt models to get the best
  cross-lingual CLIP consistency. Bets on efficiency/evolution over brute-force scaling.
  [arXiv:2508.16204](https://arxiv.org/abs/2508.16204)

## Distinctive techniques (with the receipts)

- **Magistral — pure RLVR, no cold-start.** Modified GRPO: KL penalty removed entirely
  (the policy diverges regardless, so the reference-model copy isn't worth the compute);
  Clip-Higher (ε_high tuned 0.26–0.28) to fight entropy collapse; group-relative advantage
  with minibatch normalization; non-diverse groups (all-correct/all-wrong, zero advantage)
  filtered out. A separate **language-consistency reward** (fastText classifier on
  problem/thoughts/answer) keeps reasoning in the user's language. Magistral Medium reaches
...
- **Falcon-H1 — parallel hybrid, systematically tuned.** Because attention and SSM channels
  are *concatenated* (not averaged), the team could independently sweep the attention/SSM/MLP
  channel split; more attention channels hurt loss, so they keep attention small (~1/8) and
  let Mamba-2 do most of the work (final block config is semi-parallel `SA_M`, ~2:1:5
  SSM:attn:MLP). SSM ablations: validation accuracy is driven almost entirely by larger state
  dim d_state (settled at 256) with group count nearly irrelevant; Conv1d kernel size 4 and
  chunk size 256 are sweet spots. A zero-overhead **hidden-state reset at document boundaries**
  (inject −80 into Ā before exp) stops cross-document leakage in packed sequences.
  [arXiv:2507.22448](https://arxiv.org/abs/2507.22448)
- **M2N2 — competition + attraction.** Implicit fitness sharing limits the total fitness a
  data point can supply, so models are pushed into distinct *niches* (diversity without a
...
specialize in language/regional strengths and efficiency. Korea's open-source mandate and the
THINK pruning-distillation roadmap are explicit bets on lowering the barrier to entry rather
than chasing raw scale (the THINK paper frames cost-efficiency, not benchmark wins, as the
goal). Selection pressure is real: by late 2025 the MSIT project had already cut Naver and

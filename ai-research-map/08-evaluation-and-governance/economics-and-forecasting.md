...
- **METR time-horizons** — the task length a model completes at 50% reliability has been
  ...
  ~5.3-hour horizon (320 min). The most-cited autonomy trend; METR itself cautions the metric
  may not capture the reliability/robustness real automation needs.
...
- **Frontier model-count forecast** ([arXiv:2504.16138](https://arxiv.org/abs/2504.16138),
  Kumar & Manning, GovAI/Edinburgh) — forecasts *how many AI models* will exceed the compute
  thresholds that anchor governance (EU AI Act's 10²⁵ FLOP "systemic-risk" GPAI line; the US
  AI Diffusion Framework's 10²⁶ FLOP "controlled models" line), not datacenter sizes.
  ...
  - **Key argument:** any *static* absolute threshold captures a *superlinearly* growing model
    ...
    regulators must either raise thresholds over time, scope requirements proportionally, or
    use **frontier-connected thresholds** (defined relative to the largest model to date) —
    which the paper shows stay roughly *constant* at ~14–16 models within 1 OOM of frontier
    across 2025–28.
  - **Author-stated limits:** the Notable Models dataset's notability criteria (>1000 citations,
    ...
    allocation gradient, growth rate) but *not* this selection effect — only ~6 historical
...
- **AI 2027** (Kokotajlo et al.) — influential scenario; lead author Daniel Kokotajlo has said
  ...
  ~2030 (vs. ~2028 by the time the document was finished), with autonomous coding and
  superintelligence milestones pushed toward the early 2030s. [AI 2027](https://ai-2027.com/) ·
...
2027's lead author now ~2030). Time-horizon metrics may not capture reliability/robustness
needed for real automation (METR's own caveat). Labor data is early and
augmentation-vs-displacement is unresolved. Scaling laws predict loss, not capabilities or
economic value. Selection/measurement effects abound — e.g. Kumar & Manning's counts rest on
Epoch's *notable*-models subset, biasing medians to a lower bound, and any static absolute
compute threshold captures a *superlinearly* growing set of models, pressuring
frontier-relative thresholds instead.

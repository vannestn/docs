...
- **Darwin Gödel Machine (DGM)** — rewrites its own agent code (the tools and workflows
  wrapped around a *frozen* foundation model), tests each rewrite on a coding benchmark, and
  keeps an evolutionary **archive** of every agent it generates — exploring broadly rather than
  greedily hill-climbing. Over 80 iterations it raised its own SWE-bench Verified score
  **20.0%→50.0%** and Polyglot **14.2%→30.7%** (full benchmark; the 50-task subset went
...
- **Absolute Zero Reasoner (AZR)** — a *single* model acts as both proposer and solver: it
  invents its own code-reasoning tasks (across **abduction / deduction / induction**) and solves
  them, using a Python executor as the only verifier — zero external data. The proposer is
  rewarded for *learnability* (`r_propose = 1 − r̄_solve` when 0<r̄<1, else 0): tasks that are
  trivially solved or unsolvable earn no reward, so the model builds its own curriculum aimed at
  the frontier of its ability — a concrete instance of the "learnable info gain" principle below.
  AZR-Coder-7B
...
- A unifying **Generator–Verifier–Updater (GVU) operator** casts AlphaZero, GANs, STaR,
...
- **SIMA 2** (Gemini Flash-Lite agent) self-improves inside held-out games and **Genie 3**-
  generated worlds with no new human data: a Gemini **task setter** invents instructions, a
  Gemini **reward model** (universal reward function, 0–100 rubric, ≥50 = success) scores the
  resulting rollouts, and the agent trains on this self-generated, self-scored experience. This
  roughly *doubles* SIMA 1's success rate (≈33%→66% human-eval, 30%→66% auto-eval), approaching
  human (~76–86%). On held-out ASKA the loop pushes average score past the human reference over
  successive iterations and lets the agent climb the tech tree far beyond the static SIMA 2; on
  Genie 3, self-improving on urban train tasks transfers to held-out natural test tasks — an AI
  learning in worlds another AI invents. Limitations: long-horizon tasks, short memory, and
...
- The **AI-GA / open-endedness** program is the intellectual frame. *"Open-Endedness is
  ...
  observer — novelty alone is white noise, learnability alone is static. It argues today's
  foundation models are *not* open-ended (a fixed, learnable dataset is eventually fully
  modeled), but that FMs plus open-ended algorithms are the path to ASI. [arXiv:2406.04268](https://arxiv.org/abs/2406.04268)
...
- **AI Scientist-v2** (Sakana) — end-to-end agentic discovery via a **progressive agentic
  ...
  6/6/7, ~top 45%), clearing the acceptance threshold — the **first fully AI-generated paper to
  pass peer review** (on compositional regularization; reported *negative* results). The paper
  was withdrawn pre-publication by agreement with ICLR leadership. Author-flagged limitations:
  occasional **citation hallucination** and lack of main-conference rigor.
...
novel algorithmic results, and DGM's ablations show open-ended exploration and self-improvement

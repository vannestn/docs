...
When allocated well — through verifier search plus adaptive response distributions —
test-time compute can beat parameter scaling per FLOP. That is the foundational result
from UC Berkeley / Google DeepMind, *"Scaling LLM Test-Time Compute Optimally can be More
Effective than Scaling Model Parameters"* ([arXiv:2408.03314](https://arxiv.org/abs/2408.03314)),
which reframed reasoning as a *training-time + inference-time* co-design problem.

**What the paper actually shows** (Snell, Lee, Xu, Kumar; PaLM 2-S\* on MATH 500): there
are two knobs for scaling inference — (1) *searching* against a dense process verifier
(a process reward model, or PRM), and (2) iteratively *revising* the model's own answers
(modifying the proposal distribution). Which knob works best depends on prompt difficulty,
so they fit a **compute-optimal** policy that picks the strategy per prompt using a
*model-predicted difficulty* bin (the model's own pass@1 over 2048 samples, sorted into 5
levels — no oracle label needed). Two headline results, both qualified:
...
- On search: beam search helps most at *low* budgets and on *harder* questions; at high
  ...
  most powerful method tried (lookahead/MCTS-style search) fares worst of all because of
  its extra rollout cost. Inducing the revision/verification abilities in the base model
  required capability-specific finetuning.
...
- **GRPO** (Group Relative Policy Optimization) — replaces PPO's value critic with
  group-relative advantages (each sampled group's rewards are normalized into an advantage)
  — became the default RL-for-reasoning algorithm. Successors refine it: **GSPO** (Qwen)
  lifts importance sampling/KL to the sequence level to cut gradient variance and fix
  GRPO's length bias; **CISPO** (MiniMax) and **Clip-Cov/KL-Cov** (entropy-collapse
  mitigations) belong to the same lineage. Magistral and DeepSeek-V3.2 (below) add their
  own GRPO modifications — Clip-Higher, KL removal, loss normalization, off-policy sequence
  masking — showing the algorithm is still being actively re-engineered for stable RL
  scaling. See
...
- **GenPRM** — reframes the process reward model as a *generative* task: instead of a scalar
  value head, it emits an explicit chain-of-thought (CoT) rationale **plus executable code
  verification** for each reasoning step, then judges Yes/No (Eqs. 4–5). It is trained on
  only **23K** synthesized examples from MATH (via Relative Progress Estimation + consensus
  filtering). Because the verifier itself reasons, its accuracy *scales with its own
  test-time compute* (majority vote over N verification paths). Results on ProcessBench (F1):
  a **1.5B** GenPRM (Maj@8) outperforms **GPT-4o** (61.9 → 63.4), and a **7B** GenPRM (Maj@8,
  80.5) surpasses the **Qwen2.5-Math-PRM-72B** (78.3) it is ~10× smaller than. As a *critic*
  for iterative refinement it beats Self-Refine and DeepSeek-R1-Distill-7B, giving **3.4×**
  larger gains over 3 refinement turns. The gains from verifier test-time scaling are larger
  on *harder* problems.
...
- **Magistral** (Mistral) — Mistral's first reasoning model, built from the ground up on
  their own scalable RLVR/GRPO stack. **Magistral Medium** is trained by **pure RL on
  Mistral Medium 3, with no distillation and no cold-start reasoning traces** — a ~50%
  AIME'24 boost to **73.6% pass@1 (90.0% maj@64)**. The open-weight **Magistral Small (24B,
  Apache 2.0)** adds cold-start SFT from Medium traces, then RL. Notable finding (⚠️ *contra*
  DeepSeek-R1): RL *can* substantially beat the distillation-SFT baseline even for a small
  model (SFT+RL > SFT or RL alone). RL on text-only data preserved or improved multimodal,
  instruction-following, and tool-calling ability. [arXiv:2506.10910](https://arxiv.org/abs/2506.10910)
...
- **DeepSeek-V3.2** — combines three pieces: **DeepSeek Sparse Attention (DSA)**, which
  cuts core-attention cost from O(L²) to O(Lk) via a lightning indexer plus top-k token
  selection; a scalable GRPO-based RL framework; and a **"thinking in tool-use"** retention
  mechanism (keep CoT across tool calls; discard it only on a new *user* message). Base V3.2
  is on par with GPT-5. The high-compute variant **DeepSeek-V3.2-Speciale** (RL on reasoning
  data only, reduced length penalty, + DeepSeekMath-V2 proof data) is the one that
  **surpasses GPT-5, is on par with Gemini-3.0-Pro, and reports gold-medal performance in
  IMO 2025, CMO 2025, IOI 2025, and ICPC World Final 2025**. [arXiv:2512.02556](https://arxiv.org/abs/2512.02556)

## State of research

**Best-performing now:** RL on verifiable rewards (the RLVR/GRPO lineage) is the proven recipe for
math/code reasoning, deployed across every frontier lab; unified fast/thinking routers (GPT-5) and
parallel-hypothesis search (Deep Think) are now shipped.

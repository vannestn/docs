...
| **Shortlist gate** ⬅ *we are here* | CH-1 + CH-5 from the ledger; use the **scored [comparison index](05-domain-comparison-index.md)** as the side-by-side evidence (rule: a row whose *Confidence* score is ≤2 may NOT be shortlisted **or rejected** on its scores alone — it gets a deeper brief first); apply selection criteria at coarse grain; convert Cluster-A funding unknowns from "website says" to "human confirmed" for shortlisted fields only | Stage 0 done + index scored | A written shortlist of **3–5 fields**, each with a one-paragraph impact thesis and a named candidate gap; rejected fields get one honest sentence each |
...
5. **Cost & feasibility arithmetic** (ledger A-3, U-E1–E4). Classify the candidate study: **re-analysis tier** (verified ≤$500, API/public-data only) or **primary-data tier** (needs a grant — park until funding is confirmed). Include IRB, ToS/legal exposure, and data-access checks — **plus the full resource profile**: hardware class required (API-only / free-tier T4 / consumer GPU / rented A100-hours at *current* prices); **experiment iteration speed** vs the part-time clock (a study whose single run takes days fails ≤15 hrs/wk pacing regardless of cost); **data quality & availability** (open vs gated/ToS-bound, label noise, contamination, staleness); and **supply/price exposure** (GPU rental volatility, API price or quota changes mid-study — pin prices at design time, re-check at start).
